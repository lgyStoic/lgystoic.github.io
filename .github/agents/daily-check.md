...
写文件 `radar/checks/YYYY-MM-DD.md`（同一天重复巡检就覆盖），**不超过 8 行**，和代码改动一起 commit、push。模板：
...
没有任何问题时也要留一条，只写第一、二行。会话的最终回复不超过 3 行，只有「AI 连续两天退回规则」「定时连续两天没跑」「需要 Garry 拍板」三种情况才以「⚠️ 需要关注」开头——这一行会推送到手机。

`/status/` 页面（`tools/build.py` 渲染）会展示最近 14 天的报告和最近一次运行的源健康表，Garry 在手机上看的是那里。
