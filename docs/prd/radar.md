...
| 每期页 `/radar/<日期>/` | 统计行（条数、源通率、模式 / 模型）、失败源折叠、只保留高优先级 ≤10 与中优先级 ≤20 条；低优先级与超限条目直接丢弃、记入 seen，避免重复出现；活动清单一行指引（不再内嵌活动列表）、RSS 链接 |
...
5. `finalize` / `sort_rows` → 仅保留 high 前 10、medium 前 20，低优先级及超限条目丢弃并写入 `seen.json`；再解析 Google News 中转链接（每轮最多 30 篇，HTTP 429 后停止并留待下次运行），写 `radar/data/<日期>.json`。
