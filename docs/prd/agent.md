...
AI 模型：`call_llm_json` 走 Claude（有 key）否则 Gemini；模型名由仓库变量 `vars.GEMINI_MODEL` / `vars.GEMINI_FALLBACK_MODEL` 覆盖，默认别名 `gemini-pro-latest`（→3.1 Pro）/ `gemini-flash-latest`（→3.8 Flash）；可用模型清单看 site.md §5 或跑 `xhs.yml list_models`。日志出现 `finishReason=MAX_TOKENS` → 调大 `GEMINI_MAX_OUTPUT_TOKENS`。
