...
|---|---|---|---|---|---|---|
| 雷达 | `radar.py main` | `radar/sources.json`（site/categories/rules/sources；源字段 kind,json,urls(镜像),link_host,weight,window_hours,title_pattern,require_topic,disabled；Google News 链接在采集时解析成原文地址，每轮限 30 篇并对 429 熔断；X 抓不到，人物走 Bluesky RSS；趋势源必须带创建日期；URL 支持 {today-Nd}；加源前跑 probe.yml） | `radar/data/<日期>.json`, `seen.json` | radar.yml 23:50/00:20/02:05 UTC + guard | `collect(match_tracks), decode_google_news_link, score_item, dedupe_titles, enrich_with_ai, finalize` | `[radar]`, `[radar-ai]` |
